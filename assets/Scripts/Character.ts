...
export default class Character extends cc.Component {
    ...
    @property
    controlledByPlayer: boolean = false;
    ...
    start() {
        ...
        cc.systemEvent.on('mousemove', this.onMouseMove, this);

        cc.systemEvent.on('mousedown', this.onMouseDown, this);
        ...
        this.node.on('endcutscene', this.onCutsceneEnd, this);
    ...
    onKeyUp(_event: cc.Event.EventKeyboard) {
        if (!this.dead && this.controlledByPlayer) {
        ...
        if (!this.dead && this.controlledByPlayer) {
            if (_event.keyCode == cc.macro.KEY.w) {
                ...
                if (this.weaponNode != null && this.weapon != null) {
                    ...
                    if (armature.flipX) {
                        if (this.weapon.Fire(cc.v2(armature.getBone("gun").global.x + this.node.getPosition().x - 50, -armature.getBone("gun").global.y + this.node.getPosition().y), 180, this.node)) {
                        ...
                        if (this.weapon.Fire(cc.v2(armature.getBone("gun").global.x + this.node.getPosition().x + 50, -armature.getBone("gun").global.y + this.node.getPosition().y), 0, this.node)) {
    ...
    onDamage(damager: cc.Node, amount: number): void {
        if (!this.dead) {
            if (cc.audioEngine.getState(this.hurtSoundChannelId) != cc.audioEngine.AudioState.PLAYING) {
                let num: number = Math.floor(Math.random() * (this.hurtSound.length));

                if (this.hurtSound[num] != null) {
                    this.hurtSoundChannelId = cc.audioEngine.playEffect(this.hurtSound[num], false);
                }
            }


            this.health -= amount;
            if (this.health < 0) { this.health = 0; this.dead = true; this.die();}
        }
    }
    ...
    update(dt) {
        ...
        if (!this.isPaused) {
            ...
            if (this.cameraNode != null) {
                ...
                //if (this.cameraNode.is3DNode) {
                this.cameraNode.z += 1000;
                this.cameraNode.y += 200;
                //}
            }
